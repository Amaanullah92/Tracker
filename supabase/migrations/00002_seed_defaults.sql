-- Seed default habits (idempotent: checks existence by name + user_id pattern)
-- Since this runs before any user exists, we use a service-role pattern.
-- These become templates that get copied on user signup.

-- We'll insert seed data via a function that's called after user creation.
-- First, create the seed function:
...
BEGIN
  ...
  -- 1. Namaz (shared 3-field template, composed as 5 sub-cards, special completion logic)
  ...
  -- 9. Money (any logged values = done, even $0 is a valid entry)
  INSERT INTO habits (user_id, name, description, field_schema, completion_field, completion_value, streak_direction, sort_order, is_active)
  VALUES
  ...
  -- Mark Tricep Pushdown + Reverse-Grip as a superset pair
...
-- Create a trigger to auto-seed defaults on user signup
...
AS $$
BEGIN
  PERFORM seed_defaults_for_user(NEW.id);
  RETURN NEW;
END;
$$;
