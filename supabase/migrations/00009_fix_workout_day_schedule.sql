-- Fix workout day schedule: shift from Sunday-start to Monday-start.
-- Old: Sun(0)→Push1, Mon(1)→Pull1, Tue(2)→Shoulder+Legs, Wed(3)→Push2, Thu(4)→Pull2, Fri(5)→Legs
-- New: Mon(1)→Push1, Tue(2)→Pull1, Wed(3)→Shoulder+Legs, Thu(4)→Push2, Fri(5)→Pull2, Sat(6)→Legs
--       Sun(0) is rest day (no template)

UPDATE workout_days SET scheduled_weekday = scheduled_weekday + 1;
...
BEGIN
  ...
  VALUES
    (p_user_id, 'Namaz', '5 daily prayers – Fajr, Zuhr, Asr, Maghrib, Isha',
      '[{"key": "status", "label": "Status", "type": "select", "options": ["Prayed", "Not Prayed"]},{"key": "jamat", "label": "With Jamat?", "type": "toggle"},{"key": "completeness", "label": "Completeness", "type": "select", "options": ["Full", "Partial", "Farz Only"]}]'::jsonb,
      'status', 'Prayed',
      'positive', 1, true);
  ...
  WITH pd1 AS (
    ...
    VALUES (p_user_id, 'Push Day 1', 1, 1)
    RETURNING id
  )
  ...
  -- Pull Day 1 (Tuesday = 2)
  WITH pd1 AS (
    INSERT INTO workout_days (user_id, name, scheduled_weekday, sort_order)
    VALUES (p_user_id, 'Pull Day 1', 2, 2)
    RETURNING id
  )
  INSERT INTO workout_day_exercises (workout_day_id, exercise_id, sort_order)
  SELECT pd1.id, e.id, row_number() OVER ()
  ...
  -- Shoulder + Legs (Wednesday = 3)
  WITH sl AS (
    INSERT INTO workout_days (user_id, name, scheduled_weekday, sort_order)
    VALUES (p_user_id, 'Shoulder + Legs', 3, 3)
    RETURNING id
  )
  INSERT INTO workout_day_exercises (workout_day_id, exercise_id, sort_order)
  ...
  -- Push Day 2 (Thursday = 4)
  WITH pd2 AS (
    INSERT INTO workout_days (user_id, name, scheduled_weekday, sort_order)
    VALUES (p_user_id, 'Push Day 2', 4, 4)
    RETURNING id
  )
  INSERT INTO workout_day_exercises (workout_day_id, exercise_id, sort_order)
  ...
  WITH pd2 AS (
    INSERT INTO workout_days (user_id, name, scheduled_weekday, sort_order)
    VALUES (p_user_id, 'Pull Day 2', 5, 5)
    RETURNING id
  )
  INSERT INTO workout_day_exercises (workout_day_id, exercise_id, sort_order)
  SELECT pd2.id, e.id, row_number() OVER()
