-- Fix: handle_new_user has SET search_path = '' which prevents finding
-- unqualified public.seed_defaults_for_user().
-- Also fix seed_defaults_for_user which has the same issue for unqualified
-- table references.
-- Fix: change search_path from '' to 'public' so all public schema
...
-- seed_defaults_for_user also has SET search_path = '' and references
-- tables like habits, exercises, workout_days without qualification
